...
const envSchema = z
  ...
  .superRefine((value, context) => {
    if (!value.AZURE_TENANT_ID?.trim() && !value.MICROSOFT_TENANT_ID?.trim()) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        message: 'AZURE_TENANT_ID or MICROSOFT_TENANT_ID is required',
        path: ['AZURE_TENANT_ID'],
      });
    }

    if (!value.AZURE_CLIENT_ID?.trim() && !value.MICROSOFT_CLIENT_ID?.trim()) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        message: 'AZURE_CLIENT_ID or MICROSOFT_CLIENT_ID is required',
